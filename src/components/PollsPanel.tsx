import { Button, Stack, TextInput, Text, Progress } from "@mantine/core";
...
function PollsPanel({
  ...
  const create = () => {
    ...
    onCreate(question.trim(), [optA.trim(), optB.trim()]);
    setQuestion("");
    setOptA("");
    setOptB("");
  };
  ...
  return (
    <div className="h-[85vh] pt-2 flex flex-col gap-4 overflow-y-auto">
      {isAdmin && (
        <Stack spacing="xs" className="border-b pb-3">
          ...
          <TextInput
            ...
            value={optA}
            onChange={(e) => setOptA(e.currentTarget.value)}
          />
          <TextInput
            placeholder="Option B"
            value={optB}
            onChange={(e) => setOptB(e.currentTarget.value)}
          />
      ...
      {polls.map((poll) => {
        ...
        return (
          <div key={poll.id} className="border rounded-lg p-3 flex flex-col gap-2">
